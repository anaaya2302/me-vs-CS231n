...
The first Linear Classifier was made by Ronald A. Fisher in 1936 to make a decision boundary to separate the flower varieties Iris setosa and Iris versicolor. [^2] Unlike modern classifiers, his classifier did not learn an ideal weight matrix; rather, he caclulated it manually using statistical formulae. But, this is relevant because we can see how it acts like a modern linear classifier.   

Visualising his classifier leads to the interpretation as follows: Each axis represents a real physical value. In his case, the four axes were sepal width, sepal length, petal width and petal length. The mean projected values when caclulated using the weight vector for setosa, came out to be approximately 12cm for setosa and -21cm for versicolor. (This +12 shows that the 'ideal' Iris setosa flower strongly coincides with the calculated weight for the same. And the -21 for versicolor shows that the 'ideal' versicolor is in the direction opposite the weight matrix for the ideal setosa.) We can appreciate how opposing the the ideal weight vectors would have to be to get such opposing values. Or in other words, the linear decision boundaries, will be distinct but close in space thus creating a clear separation.   

<- As an aside, this was first published in a journal called The Annals of Eugenics (yikes), which then got a rebrand and became The Annals of Human Genetics. (Atleast someone got a rebrand better than jaguar... ->
[^2]: Fisher, R. A. (1936). *The use of multiple measurements in taxonomic problems.* Annals of Eugenics, 7(2), 179–188. [DOI link](https://doi.org/10.1111/j.1469-1809.1936.tb02137.x)
